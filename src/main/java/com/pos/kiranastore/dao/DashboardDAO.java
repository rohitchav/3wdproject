...
public class DashboardDAO {
    ...
    public DashboardStats getStats(String period) throws Exception {
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            String billQuery = "SELECT IFNULL(SUM(grand_total), 0) AS totalSales, "
                    + "COUNT(*) AS transactions, "
                    + "IFNULL(AVG(grand_total), 0) AS avgBill "
                    + "FROM bills WHERE " + dateCondition;

            try (PreparedStatement ps = con.prepareStatement(billQuery);
            ...
            // ✅ Items Sold
            String itemQuery = "SELECT IFNULL(SUM(bi.qty), 0) AS itemsSold "
                    + "FROM bill_items bi "
            ...
            // ✅ Purchases (Expenses)
            String purchaseQuery = "SELECT IFNULL(SUM(amount), 0) AS totalExpenses "
                    + "FROM purchases "
                    + "WHERE " + dateCondition.replace("bill_date", "date");

            try (PreparedStatement ps = con.prepareStatement(purchaseQuery);
                 ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
